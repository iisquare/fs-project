 ...
 *
 * @example
 * <data-schema-text v-model="fields" v-model:types="['String', 'Integer', 'Boolean', 'Date']" />
 */
import { ref } from 'vue';
import DataSchemaTable from './DataSchemaTable.vue';
...
const handleText = () => {
  editorRef.value.setContent(model.value.map((item: any) => {
    return [item.name, item.title, item.type, item.comment].join(separator.value)
  }).join('\n'))
}
const handleTable = () => {
...
<template>
  ...
  <CodeEditor ref="editorRef" v-model="schema" :height="200" mode="javascript" resizable />
  <DataSchemaTable v-model="model" :types="types" />
</template>
