...
const handleEmail = () => {
  formRef.value && formRef.value.validate(valid => {
    ...
    RbacApi.forgot(param, { success: true }).then(result => {
      form.value.verify = ''
      handleTime(120)
    }).catch(() => {
...
const handleSubmit = () => {
  formRef.value && formRef.value.validate(valid => {
    ...
    loading.value = true
    RbacApi.forgot(form.value, { success: true }).then(result => {
      router.push({
        path: '/login',
        query: route.query,
      })
  ...
  <div class="container">
    ...
    <el-form ref="formRef" :model="form" :rules="rules" size="large" class="login-form">
      ...
      <el-form-item prop="password">
        <el-input v-model="form.password" :prefix-icon="ElementPlusIcons.Lock" placeholder="重置密码" type="password" show-password />
      </el-form-item>
      ...
      <el-form-item>
        <el-button type="primary" :loading="loading" @click="handleSubmit" class="login-button" :disabled="!form.verify">重置密码</el-button>
      </el-form-item>
