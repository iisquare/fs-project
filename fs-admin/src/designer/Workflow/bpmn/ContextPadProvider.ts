...
// 同 diagram-js/lib/util/Mouse#hasPrimaryModifier，避免直接依赖 diagram-js
const hasPrimaryModifier = (event: any) => {
  const originalEvent = event.originalEvent || event
  if (!originalEvent) return false
  const ctrlKey = originalEvent.ctrlKey || originalEvent.metaKey
  return !!ctrlKey && !originalEvent.altKey
}

/**
 * 上下文面板提供者 - 提供节点连线、删除等操作入口
 */
export default class ContextPadProvider {
  ...
  constructor (config: any, injector: any, eventBus: any, contextPad: any, modeling: any,
    elementFactory: any, connect: any, create: any, popupMenu: any, canvas: any, rules: any, translate: any) {
    config = config || {}
  ...
  getContextPadEntries (element: any) {
    ...
    let deleteAllowed: any = rules.allowed('elements.delete', { elements: [element] })
