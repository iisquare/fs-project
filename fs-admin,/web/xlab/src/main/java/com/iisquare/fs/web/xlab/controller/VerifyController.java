...
import com.iisquare.fs.web.xlab.tensorflow.TensorFlowService;
...
public class VerifyController extends ControllerBase {
    ...
    @Autowired
    private MrcnnService mrcnnService;
    ...
    public String roadAction(@RequestBody Map<?, ?> param) {
        ...
        List<Point> line = verifyService.road(result, step, debug ? image : null);
        if (debug) {
            return ApiUtil.echoResult(0, null, DPUtil.buildMap(
                    "line", line, "param", param, "result", cvService.imencode(image, true)
            ));
        }
