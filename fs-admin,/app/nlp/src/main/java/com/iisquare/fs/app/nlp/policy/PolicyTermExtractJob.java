...
public class PolicyTermExtractJob {
    public static void main(String[] args) {
        SparkConf config = DemoConfig.spark().setAppName(PolicyTermExtractJob.class.getSimpleName());
        SparkSession session = SparkSession.builder().config(config).getOrCreate();
        Dataset<Row> dataset = DemoSource.mysql(session, NLPCore.SQL_POLICY, 500);
        ...
        DemoSink.elasticsearch(dataset, "fs_policy_term", "id", 200);
        session.close();
    }
}
