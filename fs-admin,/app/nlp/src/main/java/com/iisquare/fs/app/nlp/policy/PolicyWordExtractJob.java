package com.iisquare.fs.app.nlp.policy;
...
public class PolicyWordExtractJob {
    public static void main(String[] args) {
        SparkConf config = DemoConfig.spark().setAppName(PolicyWordExtractJob.class.getSimpleName());
        SparkSession session = SparkSession.builder().config(config).getOrCreate();
        // 获取政策标题和内容
        Dataset<Row> dataset = DemoSource.mysql(session, NLPCore.SQL_POLICY_SAMPLE, 500);
        // 清除政策内容的HTML标签
        ...
        DemoSink.mysql(dataset, SaveMode.Overwrite, "fs_policy_sdp", 200);
        session.close();
    }
}
