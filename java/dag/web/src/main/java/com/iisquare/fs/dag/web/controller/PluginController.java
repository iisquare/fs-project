...
import com.iisquare.fs.dag.web.service.DagService;
import com.iisquare.fs.dag.web.service.PluginService;
...
public class PluginController extends PermitControllerBase {
    ...
    @Autowired
    private DagService sparkService;
    @Autowired
    ...
    public String treeAction(@RequestParam Map<?, ?> param) {
        return DPUtil.stringify(pluginService.tree(new File(sparkService.pluginsDir())));
    }
    ...
    public String uploadAction(HttpServletRequest request, @RequestPart("file") MultipartFile file) {
        ...
        String pluginsDir = sparkService.pluginsDir();
        if(file.getOriginalFilename().toLowerCase().endsWith(".jar")) { // 主JAR文件
            try {
                File jar = new File(pluginsDir + "/app.jar");
        ...
        if(pluginService.exists(pluginName) || FileUtil.isExists(pluginsDir + "/" + pluginName)) {
            return ApiUtil.echoResult(1007, "插件已存在，请卸载后重新安装", null);
        }
        for (Map.Entry<String, List<Integer>> entry : zipfiles.entrySet()) {
            String name = entry.getKey();
            String filepath = pluginsDir + "/" + pluginName + "/" + name;
        ...
        if(!FileUtil.putContent(pluginsDir + "/" + pluginName + "/config.json", json, "utf-8")) {
    ...
    public String deleteAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
        ...
        String filepath = sparkService.pluginsDir() + "/" + info.getName();
