...
import com.iisquare.fs.web.admin.service.FlowPluginService;
...
@RestController
@RequestMapping("/flowPlugin")
@RefreshScope
public class FlowPluginController extends PermitController {

    @Autowired
    private FlowPluginService flowPluginService;
    @Value("${custom.flow.plugins.path}")
    private String flowPluginsPath;

    @GetMapping("/tree")
    @Permission("")
    public String treeAction(@RequestParam Map<?, ?> param) {
        return DPUtil.stringify(flowPluginService.tree(new File(flowPluginsPath)));
    }

    @RequestMapping("/list")
    @Permission("")
    public String listAction(@RequestBody Map<?, ?> param) {
        Map<?, ?> result = flowPluginService.search(param, DPUtil.buildMap(
    ...
    public String uploadAction(HttpServletRequest request) {
        ...
        if(file.getOriginalFilename().toLowerCase().endsWith(".jar")) { // 主JAR文件
            try {
                File jar = new File(flowPluginsPath + "/app.jar");
        ...
        if(flowPluginService.exists(pluginName) || FileUtil.isExists(flowPluginsPath + "/" + pluginName)) {
            return ApiUtil.echoResult(1007, "插件已存在，请卸载后重新安装", null);
        }
        for (Map.Entry<String, List<Integer>> entry : zipfiles.entrySet()) {
            String name = entry.getKey();
            String filepath = flowPluginsPath + "/" + pluginName + "/" + name;
    ...
    public String saveAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
        ...
        if(!flowPluginService.status("default").containsKey(status)) return ApiUtil.echoResult(1002, "状态异常", status);
        FlowPlugin info = flowPluginService.info(id);
    ...
    public String deleteAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
        ...
        FlowPlugin info = flowPluginService.info(ids.get(0));
        if(null == info || -1 == info.getStatus()) return ApiUtil.echoResult(404, null, info);
        String filepath = flowPluginsPath + "/" + info.getName();
        if(!FileUtil.delete(filepath, true)) {
            return ApiUtil.echoResult(5001, "文件删除失败", filepath);
        }
        boolean result = flowPluginService.delete(ids, uid(request));
    ...
    public String configAction(ModelMap model) {
        model.put("status", flowPluginService.status("default"));
