...
@Service
public class NodeService extends ServiceBase implements DisposableBean, ApplicationListener<WebServerInitializedEvent> {
    ...
    public ObjectNode state() throws Exception {
        ...
        ObjectNode schedule = state.putObject("scheduler");
        schedule.put("id", scheduler.getSchedulerInstanceId());
        schedule.put("name", scheduler.getSchedulerName());
        schedule.put("standby", scheduler.isInStandbyMode());
        schedule.put("started", scheduler.isStarted());
        schedule.put("shutdown", scheduler.isShutdown());
        return state;
    }
    ...
    private Scheduler newScheduler() throws Exception {
        SchedulerFactory factory = new StdSchedulerFactory(
                PropertiesUtil.load(NodeService.class.getClassLoader(), "quartz.properties"));
