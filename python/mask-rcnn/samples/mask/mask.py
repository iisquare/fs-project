...
Usage: run from the command line as such:
    ...
    python mask.py train --dataset=/path/to/mask/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python mask.py train --dataset=/path/to/mask/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python mask.py train --dataset=/path/to/mask/dataset --weights=imagenet

    # Detect mask from image
    python mask.py detect --weights=/path/to/weights/file.h5 --image=<URL or path to file>
...
FULL_CLASS_NAMES = [
    'BG', '58_br', 'anjuke_br', '5i5j_wm', '58_wm', 'anjuke_wm', 'centanet_ct', 'fang_br', 'ke_wm', 'lianjia_wm']
...
class MaskDataset(utils.Dataset):
    ...
    def load_dataset(self, dataset_dir, subset):
        ...
        for index in range(1, len(FULL_CLASS_NAMES)):
            self.add_class("mask", index, FULL_CLASS_NAMES[index])
        ...
        for class_dir in os.listdir(dataset_dir):
            ...
            for file_name in os.listdir(class_path):
                ...
                if names[-1] != 'jpg':
                    continue
                regions = []
                region_path = os.path.join(class_path, '.'.join(names[:-1]))
                ...
                self.add_image(
                    "mask",
                    image_id='{}_{}'.format(class_dir, file_name),  # use file name as a unique image id
                    path=file_path,
