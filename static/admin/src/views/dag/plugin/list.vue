  ...
  <section>
      ...
      <div class="table-page-search-wrapper">
        ...
        >
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'dag:plugin:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'dag:plugin:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
            </a-button-group>
          </span>
        </a-table>
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'dag:plugin:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'dag:plugin:add'" />
          <a-upload
            v-permit="'dag:plugin:add'"
    ...
    <a-modal :title="'信息查看 - ' + form.id" v-model="infoVisible" :footer="null">
      <a-form-model :model="form" :loading="infoLoading" :label-col="{ span: 5 }" :wrapper-col="{ span: 15 }">
        ...
        <a-form-model-item label="配置"><a-textarea v-model="form.config" /></a-form-model-item>
...
import pluginService from '@/service/dag/plugin'
...
export default {
  data () {
    return {
      ...
      upload: {
        action: process.env.VUE_APP_API_BASE_URL + '/proxy/upload?app=dag&uri=/plugin/upload',
        loading: false
      }
    }
  },
