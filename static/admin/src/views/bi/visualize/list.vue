      ...
      <div class="table-page-search-wrapper">
        ...
        >
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'bi:visualize:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'bi:visualize:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
              <a-button type="link" size="small">
                <router-link :to="'/bi/report/chart?id=' + record.id">模型</router-link>
              </a-button>
            </a-button-group>
          </span>
        </a-table>
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'bi:visualize:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'bi:visualize:add'" />
          <a-button icon="plus-circle" type="primary" @click="add" v-permit="'bi:visualize:add'">新增</a-button>
        </div>
      </div>
    </a-card>
    <!--展示界面-->
...
export default {
  ...
  methods: {
    batchRemove () {
      this.$confirm(this.selection.confirm(() => {
        this.loading = true
        visualizeService.delete(this.selection.selectedRowKeys, { success: true }).then((result) => {
          if (result.code === 0) {
    ...
    search (filter2query, pagination) {
      ...
      this.loading = true
      visualizeService.list(this.filters).then((result) => {
    ...
    submit () {
      this.$refs.form.validate(valid => {
        ...
        visualizeService.save(data).then(result => {
          if (result.code === 0) {
  ...
  mounted () {
    this.search(false, true)
    visualizeService.config().then((result) => {
