  ...
  <section>
      ...
      <div class="table-page-search-wrapper">
        <a-form-model ref="filters" :model="filters" layout="inline">
          <a-row :gutter="48">
            ...
            <a-col :md="6" :sm="24">
              <a-form-model-item label="计算引擎" prop="engine">
                <a-select v-model="filters.engine" placeholder="请选择" :allowClear="true">
                  <a-select-option v-for="(value, key) in config.engines" :key="key" :value="key">{{ value }}</a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
            <a-col :md="6" :sm="24">
              <a-form-model-item label="处理模式" prop="model">
                <a-select v-model="filters.model" placeholder="请选择" :allowClear="true">
                  <a-select-option v-for="(value, key) in config.models" :key="key" :value="key">{{ value }}</a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
            <a-col :md="6" :sm="24">
              <span class="table-page-search-submitButtons" :style="advanced && { float: 'right', overdiagram: 'hidden' } || {} ">
        ...
        >
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'bi:diagram:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'bi:diagram:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
              <a-button type="link" size="small">
                <router-link :to="'/bi/diagram/model?id=' + record.id" target="_blank">模型</router-link>
              </a-button>
            </a-button-group>
          </span>
        </a-table>
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'bi:diagram:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'bi:diagram:add'" />
          <a-button icon="plus-circle" type="primary" @click="add" v-permit="'bi:diagram:add'">新增</a-button>
        </div>
      </div>
    </a-card>
    <!--展示界面-->
    <a-modal :title="'信息查看 - ' + form.id" v-model="infoVisible" :footer="null">
      <a-form-model :model="form" :loading="infoLoading" :label-col="{ span: 5 }" :wrapper-col="{ span: 15 }">
        <a-form-model-item label="规则名称">{{ form.name }}</a-form-model-item>
        <a-form-model-item label="计算引擎">{{ form.engineText }}</a-form-model-item>
        <a-form-model-item label="处理模式">{{ form.modelText }}</a-form-model-item>
    ...
    <a-modal :title="'信息' + (form.id ? ('修改 - ' + form.id) : '添加')" v-model="formVisible" :confirmLoading="formLoading" :maskClosable="false" @ok="submit">
      <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
        ...
        <a-form-model-item label="计算引擎" prop="engine">
          <a-select v-model="form.engine" placeholder="请选择">
            <a-select-option v-for="(value, key) in config.engines" :key="key" :value="key">{{ value }}</a-select-option>
          </a-select>
        </a-form-model-item>
        <a-form-model-item label="处理模式" prop="model">
          <a-select v-model="form.model" placeholder="请选择">
            <a-select-option v-for="(value, key) in config.models" :key="key" :value="key">{{ value }}</a-select-option>
          </a-select>
        </a-form-model-item>
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        { title: '计算引擎', dataIndex: 'engineText' },
      ...
      rules: {
        ...
        engine: [{ required: true, message: '请选择计算引擎', trigger: 'change' }],
  ...
  methods: {
    batchRemove () {
      this.$confirm(this.selection.confirm(() => {
        this.loading = true
        diagramService.delete(this.selection.selectedRowKeys, { success: true }).then((result) => {
          if (result.code === 0) {
    ...
    search (filter2query, pagination) {
      ...
      this.loading = true
      diagramService.list(this.filters).then((result) => {
    ...
    submit () {
      this.$refs.form.validate(valid => {
        ...
        diagramService.save(this.form).then(result => {
          if (result.code === 0) {
  ...
  mounted () {
    this.search(false, true)
    diagramService.config().then((result) => {
