  ...
  <a-tabs default-active-key="property" :animated="false">
    <a-tab-pane key="property" tab="属性">
      <a-form-model :model="value" labelAlign="left" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
        ...
        <a-form-model-item label="全量替换"><a-checkbox v-model="value.data.options.replaceDocument">整体替换文档或替换部分字段</a-checkbox></a-form-model-item>
        <a-form-model-item label="强制插入"><a-checkbox v-model="value.data.options.forceInsert">存在_id字段依然采用插入方式</a-checkbox></a-form-model-item>
      </a-form-model>
    </a-tab-pane>
  </a-tabs>
</template>
  ...
  methods: {
    formatted (obj) {
      const options = {
        ...
        replaceDocument: !!obj.data.options.replaceDocument,
        forceInsert: !!obj.data.options.forceInsert
      }
