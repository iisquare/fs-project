      ...
      <div class="table-page-search-wrapper">
        ...
        >
          ...
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'spark:flow:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'spark:flow:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
        ...
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'spark:flow:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'spark:flow:add'" />
          <a-button icon="plus-circle" type="primary" @click="add" v-permit="'spark:flow:add'">新增</a-button>
        </div>
      </div>
    </a-card>
    <!--展示界面-->
...
import flowService from '@/service/spark/flow'
