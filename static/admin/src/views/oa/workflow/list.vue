  ...
  <section>
      ...
      <div class="table-page-search-wrapper">
          ...
          <a-row :gutter="48">
            ...
            <a-col :md="6" :sm="24">
              <a-form-model-item label="表单" prop="formId">
                <a-input v-model="filters.formId" placeholder="" :allowClear="true" />
              </a-form-model-item>
            </a-col>
            <a-col :md="6" :sm="24">
        ...
        >
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'oa:workflow:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'oa:workflow:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
              <a-button type="link" size="small">
                <router-link :to="'/oa/workflow/model?id=' + record.id" target="_blank">模型</router-link>
              </a-button>
              <a-button v-permit="'oa:workflow:publish'" type="link" size="small" @click="publish(text, record)">发布</a-button>
            </a-button-group>
          </span>
        </a-table>
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'oa:workflow:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'oa:workflow:add'" />
          <a-button icon="plus-circle" type="primary" @click="add" v-permit="'oa:workflow:add'">新增</a-button>
        </div>
      </div>
    </a-card>
    <!--展示界面-->
    <a-modal :title="'信息查看 - ' + form.id" v-model="infoVisible" :footer="null">
      <a-form-model :model="form" :loading="infoLoading" :label-col="{ span: 5 }" :wrapper-col="{ span: 15 }">
        ...
        <a-form-model-item label="表单">{{ form.formId }}</a-form-model-item>
        ...
        <a-form-model-item label="发布者">{{ form.deploymentUidName }}</a-form-model-item>
        <a-form-model-item label="发布时间">{{ (form.deploymentInfo ? form.deploymentInfo.deploymentTime : 0)|date }}</a-form-model-item>
      </a-form-model>
    </a-modal>
    <!--编辑界面-->
    <a-modal :title="'信息' + (form.id ? ('修改 - ' + form.id) : '添加')" v-model="formVisible" :confirmLoading="formLoading" :maskClosable="false" @ok="submit">
      <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
        ...
        <a-form-model-item label="表单" prop="formId">
          <a-input v-model="form.formId" auto-complete="on"></a-input>
        </a-form-model-item>
  ...
  methods: {
    batchRemove () {
      this.$confirm(this.selection.confirm(() => {
        this.loading = true
        workflowService.delete(this.selection.selectedRowKeys, { success: true }).then((result) => {
          if (result.code === 0) {
    ...
    search (filter2query, pagination) {
      ...
      this.loading = true
      workflowService.list(this.filters).then((result) => {
    ...
    submit () {
      this.$refs.form.validate(valid => {
        ...
        workflowService.save(this.form).then(result => {
          if (result.code === 0) {
    ...
    publish (text, record) {
      this.loading = true
      workflowService.publish({ id: record.id }, { success: true }).then((result) => {
        if (result.code === 0) {
          this.search(false, true)
        } else {
          this.loading = false
