      ...
      <div class="table-page-search-wrapper">
        ...
        >
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'oa:print:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'oa:print:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
        ...
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'oa:print:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'oa:print:add'" />
          <a-button icon="plus-circle" type="primary" @click="add" v-permit="'oa:print:add'">新增</a-button>
        </div>
      </div>
    </a-card>
    <!--展示界面-->
...
import printService from '@/service/oa/print'
...
export default {
  ...
  methods: {
    batchRemove () {
      this.$confirm(this.selection.confirm(() => {
        this.loading = true
        printService.delete(this.selection.selectedRowKeys, { success: true }).then((result) => {
          if (result.code === 0) {
    ...
    search (filter2query, pagination) {
      ...
      this.loading = true
      printService.list(this.filters).then((result) => {
    ...
    submit () {
      this.$refs.form.validate(valid => {
        ...
        printService.save(data).then(result => {
          if (result.code === 0) {
  ...
  mounted () {
    this.search(false, true)
    printService.config().then((result) => {
