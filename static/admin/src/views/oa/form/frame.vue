      ...
      <div class="table-page-search-wrapper">
        ...
        >
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'oa:formFrame:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'oa:formFrame:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
              <a-button type="link" size="small">
                <router-link :to="'/oa/form/model?id=' + record.id" target="_blank">模型</router-link>
              </a-button>
              <a-button type="link" size="small">
                <router-link :to="'/oa/form/list?id=' + record.id" target="_blank">数据</router-link>
              </a-button>
            </a-button-group>
          </span>
        </a-table>
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'oa:formFrame:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'oa:formFrame:add'" />
          <a-button icon="plus-circle" type="primary" @click="add" v-permit="'oa:formFrame:add'">新增</a-button>
        </div>
      </div>
    </a-card>
    <!--展示界面-->
...
export default {
  ...
  methods: {
    batchRemove () {
      this.$confirm(this.selection.confirm(() => {
        this.loading = true
        formFrameService.delete(this.selection.selectedRowKeys, { success: true }).then((result) => {
          if (result.code === 0) {
    ...
    search (filter2query, pagination) {
      ...
      this.loading = true
      formFrameService.list(this.filters).then((result) => {
    ...
    submit () {
      this.$refs.form.validate(valid => {
        ...
        formFrameService.save(this.form).then(result => {
          if (result.code === 0) {
  ...
  mounted () {
    this.search(false, true)
    formFrameService.config().then((result) => {
