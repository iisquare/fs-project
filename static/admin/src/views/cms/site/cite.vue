      ...
      <div class="table-page-search-wrapper">
        ...
        >
          <span slot="action" slot-scope="text, record">
            <a-button-group>
              <a-button type="link" size="small" v-permit="'cms:cite:'" @click="show(text, record)">查看</a-button>
              <a-button v-permit="'cms:cite:modify'" type="link" size="small" @click="edit(text, record)">编辑</a-button>
            </a-button-group>
          </span>
        </a-table>
        <div :class="rows.length > 0 ? 'table-pagination-tools' : 'table-pagination-tools-empty'">
          <a-button icon="minus-circle" type="danger" @click="batchRemove" v-permit="'cms:cite:delete'" :disabled="selection.selectedRows.length === 0">删除</a-button>
          <a-divider type="vertical" v-permit="'cms:cite:add'" />
          <a-button icon="plus-circle" type="primary" @click="add(0)" v-permit="'cms:cite:add'">新增</a-button>
        </div>
      </div>
    </a-card>
    <!--展示界面-->
...
import citeService from '@/service/cms/cite'
...
export default {
  ...
  methods: {
    batchRemove () {
      this.$confirm(this.selection.confirm(() => {
        this.loading = true
        citeService.delete(this.selection.selectedRowKeys, { success: true }).then((result) => {
          if (result.code === 0) {
    ...
    search (filter2query, pagination) {
      ...
      this.loading = true
      citeService.list(this.filters).then((result) => {
    ...
    submit () {
      this.$refs.form.validate(valid => {
        ...
        citeService.save(this.form).then(result => {
          if (result.code === 0) {
  ...
  mounted () {
    this.search(false, true)
    citeService.config().then((result) => {
