...
public class TaskController extends PermitControllerBase {
    ...
    @GetMapping("/nodes")
    public String nodesAction(@RequestParam Map<String, String> param, HttpServletRequest request) {
        ...
        for (String node : taskService.participants()) {
            ...
            String content = HttpUtil.get(url + "/task/node", param, taskService.headers(request));
    ...
    @Permission("")
    public String submitAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
        ...
        int prefetchCount = ValidateUtil.filterInteger(param.get("prefetchCount"), 0, 500, 1);
        if (prefetchCount < 0) return ApiUtil.echoResult(1002, "预先载入数量异常", prefetchCount);
        int consumerCount = ValidateUtil.filterInteger(param.get("consumerCount"), 0, null, 1);
    ...
    @Permission("")
    public String startAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
        String queueName = DPUtil.trim(DPUtil.parseString(param.get("queueName")));
        if (DPUtil.empty(queueName)) return ApiUtil.echoResult(1001, "队列名称不能为空", queueName);
    ...
    @RequestMapping("/stop")
    @Permission("")
    public String stopAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
        String queueName = DPUtil.trim(DPUtil.parseString(param.get("queueName")));
        if (DPUtil.empty(queueName)) return ApiUtil.echoResult(1001, "队列名称不能为空", queueName);
    ...
    @Permission("")
    public String rebalanceAction(@RequestBody Map<String, String> param, HttpServletRequest request) {
    ...
    @Permission("")
    public String startAllAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
    ...
    @Permission("")
    public String stopAllAction(@RequestBody Map<?, ?> param, HttpServletRequest request) {
